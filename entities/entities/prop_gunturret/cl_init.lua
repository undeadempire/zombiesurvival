...
function ENT:Draw()
	...
	--[[if self.WeaponString ~= "" then 
		local weapon = self.WeaponString 
		local wid, hei = 128, 108
	end]]
	...
	cam_Start3D2D(self:LocalToWorld(self.VScreen), self:LocalToWorldAngles(aScreen), 0.075)
		...
		--[[if self.WeaponString ~= "" then
			local pdata = self.Weaponpdata
			local wepname = pdata.PrintName
			draw_SimpleText("Weapon:"..wepname, "DefaultFontBold", x, 55, COLOR_WHITE, TEXT_ALIGN_CENTER)
		end]]
