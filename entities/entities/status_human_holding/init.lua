...
concommand.Add("_zs_rotateang", function(sender, command, arguments)
	...
	local snap = tonumbersafe(arguments[3])
...
function ENT:Think()
	...
	elseif owner:KeyDown(IN_ATTACK2) and not owner:GetActiveWeapon().NoPropThrowing then
		...
		elseif owner:KeyDown(IN_WALK) then
			...
			-- snap to all 3 rotational axis
			if owner.snap != 0 and owner:KeyDown(IN_RELOAD) then
				self.ObjectAngles.p = self.ObjectAngles:SnapTo("p", owner.snap).p
				self.ObjectAngles.y = self.ObjectAngles:SnapTo("y", owner.snap).y
				self.ObjectAngles.r = self.ObjectAngles:SnapTo("r", owner.snap).r
			end
