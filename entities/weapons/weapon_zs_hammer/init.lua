...
function SWEP:SecondaryAttack()
	...
	or trent:IsNailed() and (#trent.Nails >= 100 or trent:GetPropsInContraption() >= GAMEMODE.MaxPropsInBarricade)
	...
	for _, nail in pairs(ents.FindByClass("prop_nail")) do
		if nail:GetParent() == trent and nail:GetActualPos():DistToSqr(tr.HitPos) <= 15 then
	...
	or ent:IsValid() and util.IsValidPhysicsObject(ent, trtwo.PhysicsBone) and (ent:GetMoveType() == MOVETYPE_VPHYSICS or ent:GetNailFrozen()) and not ent.NoNails and not (not ent:IsNailed() and not ent:GetPhysicsObject():IsMoveable()) and not (ent:GetMaxHealth() == 1 and ent:Health() == 0 and not ent.TotalHealth) then
		...
		if ent and ent:IsValid() and (ent:IsProjectile() or ent.NoNails or ent:IsNailed() and (#ent.Nails >= 100 or ent:GetPropsInContraption() >= GAMEMODE.MaxPropsInBarricade)) then return end

--Thing that prevents destroyed props from being used
...
--Nailing animation in first person
		self:SendWeaponAnim(self.Alternate and ACT_VM_HITCENTER or ACT_VM_MISSCENTER)
		self.Alternate = not self.Alternate

--Nailing animation in third person
		owner:DoAnimationEvent(ACT_HL2MP_GESTURE_RANGE_ATTACK_MELEE)

--Thing that makes you use nails
		self:SetNextPrimaryFire(CurTime() + 1)
		self:TakePrimaryAmmo(1)

--Thing that places the nail
