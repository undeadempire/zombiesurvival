...
SWEP.PrintName = "Telekinetic Dynamo"
SWEP.Description = "Allows you to harness the power of the Shade, though you're much weaker."
...
SWEP.ShadeControl = "env_teledynamocontrol"
SWEP.ShadeProjectile = "projectile_teledynamorock"
	...
	for _, ent in pairs(ents.FindByClass(self.ShadeControl)) do
		if ent:IsValid() and ent:GetOwner() == owner then
			...
			if obj:IsValid() then
				...
				if phys:IsValid() and phys:IsMoveable() and phys:GetMass() <= 200 then
					phys:Wake()
...
function SWEP:SecondaryAttack()
	...
	local owner = self:GetOwner()
	local ent = owner:CompensatedMeleeTrace(9999, 4).Entity
	if ent:IsValid() and (ent:IsPhysicsModel() or ent.IsShadeGrabbable or ent.IsPhysbox) then
		...
		if phys:IsValid() and phys:IsMoveable() and phys:GetMass() <= 30000 then
