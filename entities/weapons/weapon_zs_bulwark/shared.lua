...
GAMEMODE:AddNewRemantleBranch(SWEP, 1, "'Citadel' Minicannon", "Uses 3 ammo per shot, shoots slower, but is precise and inflicts knockback.", function(wept)
	wept.Primary.Damage = wept.Primary.Damage * 3
	wept.ConeMin = wept.ConeMin * 0.01
	wept.ConeMax = wept.ConeMax * 0.01
	...
	wept.BulletCallback = function(attacker, tr, dmginfo)
		...
		if SERVER and trent and trent:IsValidZombie() then
			...
			if math.random(1) == 1 then
				trent:ThrowFromPositionSetZ(tr.StartPos, 80, nil, true)
			end
		end
	end
