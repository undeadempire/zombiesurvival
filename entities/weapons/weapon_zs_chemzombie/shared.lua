...
SWEP.PrintName = "Chemical Amalgam"
...
function SWEP:MeleeHitPlayer(ent, trace, damage, forcescale) -- what happens when a zombie hits you
	ent:ThrowFromPositionSetZ(self:GetOwner():GetPos(), damage * 2.5 * (forcescale or self.MeleeForceScale)) -- Moves you by this much when you are hit in hammer units, 2.5 * damage taken * Melee forcescale, bacially what skills make you go further or zombie mod like that of the windigo
	ent:MeleeViewPunch(damage) -- viewpunch
	local nearest = ent:NearestPoint(trace.StartPos) -- Spawns blood
...
function SWEP:PrimaryAttack()
	...
	print("swung")
	if SERVER then
	function self:ApplyMeleeDamage(ent, trace, damage)
		print(ent)
		local owner = self:GetOwner()
		local pos = owner:GetPos()
		local radius = self.PrimaryRadius
		--if hitent:IsNailed() or hitent:IsValidLivingHuman() then
			for _, hitent in pairs(ents.FindInSphere(pos, radius)) do
					local dmginfo = DamageInfo()
					dmginfo:SetDamage(self.MeleeDamage)
					dmginfo:SetDamagePosition(ent:GetPos())
					dmginfo:SetDamageInflictor(self)
