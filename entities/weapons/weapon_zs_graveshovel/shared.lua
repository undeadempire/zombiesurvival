...
GAMEMODE:AddNewRemantleBranch(SWEP, 1, "Reaper Shovel", "Heals a small amount of health on kill, less damage", function(wept)
    wept.Primary.Delay = wept.Primary.Delay * 1.5
    wept.MeleeDamage = wept.MeleeDamage * 0.8
    wept.MeleeKnockBack = 375
	
	wept.OnZombieKilled = function(self, zombie, total, dmginfo)
		local killer = self:GetOwner()

		if killer:IsValid() then
			killer:HealPlayer(killer, 8)
		end
	end
