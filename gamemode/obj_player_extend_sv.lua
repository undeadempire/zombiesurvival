	...
	if P_Team(self) == TEAM_UNDEAD then
		...
		if attacker:IsValidLivingHuman() and inflictor:IsValid() and inflictor == attacker:GetActiveWeapon() then
			...
			if wep.IsMelee then
				...
				if wep.Culinary and attacker:IsSkillActive(SKILL_MASTERCHEF) and math.random(1) == 1 then
					self.ChefMarkOwner = attacker
					self.ChefMarkTime = CurTime() + 10
				end
			end
		end
...
function meta:BecomeClass(index)
    if not index then return end

    local curclass = self.DeathClass or self:GetZombieClass()
    self:SetZombieClass(index)
    self:DoHulls(index, TEAM_UNDEAD)
...
function meta:Resupply(owner, obj)
	...
	local stowage = true --self:IsSkillActive(SKILL_STOWAGE)
	...
	if not stowage then
		self.NextResupplyUse = CurTime() + GAMEMODE.ResupplyBoxCooldown * (self.ResupplyDelayMul or 1) * (stockpiling and 1 or 1)
	...
	for i = 1, stockpiling and not stowage and 2 or 1 do
		...
		if self ~= owner and owner:IsValidHuman() then
			...
			owner:AddPoints(0.6, nil, nil, true)
...
function meta:GetRandomStartingItem()
	...
	if #pool > 0 then
		...
		pool[#pool + 1] = GAMEMODE.FeliciaTrinkets[math.random(#GAMEMODE.FeliciaTrinkets)]
end
