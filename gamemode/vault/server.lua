...
function GM:ShouldSaveVault(pl)
	...
	if pl:GetZSXP() > 0 or pl:GetZSSPUsed() > 0 or pl:GetZSRemortLevel() > 0 or pl:GetZSPrestige(string) ~= "" then
		return true
	end
	...
	if file.Exists(filename, "DATA") then
		...
		if contents and #contents > 0 then
			...
			if contents then
				...
				if contents.Prestige then
					pl:SetDTString(DT_PLAYER_STRING_PRESTIGE, contents.Prestige)
				end
...
function GM:SaveVault(pl)
	...
	local tosave = {
		...
		XP = pl:GetZSXP(),
		Prestige = pl:GetZSPrestige("string"),
