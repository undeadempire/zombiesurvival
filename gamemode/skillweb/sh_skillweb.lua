...
function meta:GetZSPrestige(t)
	local prestigestring = self:GetDTString(DT_PLAYER_STRING_PRESTIGE)
	if t == "string" then
		return prestigestring
	else
		local prestigestring = string.Split(string.sub(prestigestring, 2, -3), ",")
		local prestigetable = {}

		for a,i in ipairs(prestigestring) do
			local kvp = string.Split(i, "=")
			prestigetable[kvp[1]] = tonumber(kvp[2])
		end
