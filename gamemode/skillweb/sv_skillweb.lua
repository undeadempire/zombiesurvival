...
--Shouldn't be invoked manually, only as a sub-function of AddZSPrestige.
function meta:SetZSPrestige(weapon, prestigelvl)
	local prestigetable = self:GetZSPrestige("table")
	local wepstart = string.find(weapon, "weapon_")
	if wepstart then
		--weapon = string.sub(weapon, wepstart, -2)

		if not self:GetZSPrestige("table") then
			prestigetable = {}
		end
		if not prestigetable[weapon] then
			prestigetable[weapon] = 0
		end
		prestigetable[weapon] = prestigelvl
		local prestigestring = table.ToString(prestigetable) 
		self:SetDTString(DT_PLAYER_STRING_PRESTIGE, prestigestring)
	end
end

function meta:AddZSPrestige(weapon, prestigelvl)
	local prestigetable = self:GetZSPrestige("table")
	...
	self:SetZSPrestige(weapon, prestigetable[weapon] + prestigelvl)
end
...
function meta:SkillsReset()
	self:SetUnlockedSkills({})
	self:SetDesiredActiveSkills({})
	self.NextSkillReset = os.time() + 120 -- 1 week
